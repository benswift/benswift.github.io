...
The whole thing is a single Python file---about 840 lines, with
...
which gives you a nice per-day breakdown by year. GitLab is a bit more
work---you paginate through the
[user events REST endpoint](https://docs.gitlab.com/api/events/) and count
things up yourself.
...
Caching is simple but effective: past years get saved as JSON files, and only
the current (incomplete) year gets refetched on each run. For GitHub that means
one GraphQL query per historical year on first run, then one query per
subsequent run. For GitLab it's more requests because you're paginating through
events, but the caching means you only pay that cost once.

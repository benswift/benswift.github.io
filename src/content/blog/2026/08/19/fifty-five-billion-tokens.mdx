...
description:
  "Fifteen months of agent session logs: 55 billion tokens across 14 models
  (and what they would have cost at API rates)"
tags: ["ai"]
---
...
usage numbers.[^zed] Each assistant message in there carries a `usage` block
recording what it cost to produce. Fifteen months of that comes to 23,851
transcript files and 532,027 API responses.[^dedup] Fifty-five billion
tokens---that's a big number.

But mostly I just wanted to look at it.

<Chart
  file="2026/08/19/daily-heatmap.svg"
  alt="A calendar heatmap of daily token use from May 2025 to August 2026, sparse and pale through 2025, dense and saturated from February 2026 onward"
/>
...
<Chart
  file="2026/08/19/rise-and-fall.svg"
  alt="Fourteen small-multiple area charts, one per model, where each model's daily token use rises and falls in turn as the next model replaces it"
/>

Fourteen models in fifteen months, each taking over from the last.

<Chart
  file="2026/08/19/by-project.svg"
  alt="Sixteen small-multiple area charts, one per project, each scaled to its own peak, marking out when the work on each one happened"
/>

Sixteen projects, three-quarters of the total.[^projects]

<Chart
  file="2026/08/19/cumulative.svg"
  alt="Two stacked area charts, cumulative tokens above and cumulative indicative cost below, both hockey-stick shaped with almost all of the total after March 2026"
/>

About 86% of the total sits in the last five months.

<Chart
  file="2026/08/19/diurnal.svg"
  alt="A line chart of average tokens per hour, weekdays against weekends, with a mid-afternoon peak, a dip at noon and a sharp drop at 6pm"
/>
...
<Chart
  file="2026/08/19/by-model.svg"
  alt="Two panels of horizontal bars ranking models by tokens and by indicative cost: Sonnet 5 leads on tokens but sits mid-table on cost, while Opus 4.1 has few tokens and high cost"
/>
...
<Chart
  file="2026/08/19/where-the-tokens-go.svg"
  alt="Grouped horizontal bars setting each token category's share of volume against its share of cost, with cache reads at 96% of tokens and 53% of cost"
/>
...
[^dedup]:
    ...
    transcript under two project directories, and 455 files are duplicated that
    way. The count also includes 10,253 subagent transcripts, about 21% of the
...
[^zed]:
    Zed kept per-request token counts, models and project paths for its agent
    panel, so those months needed no more than a format conversion to sit
    alongside the Claude Code transcripts. The year before _that_ --- Zed's
    older assistant panel, back to April 2024 --- kept the conversation text
    and nothing else: no usage, no model. It is in the log archive with every
    figure marked as an estimate, and it stays off these charts, which count
    only numbers something actually reported.

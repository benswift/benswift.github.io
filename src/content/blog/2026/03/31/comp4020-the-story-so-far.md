...
art school. Each week follows the same cycle:
...
ripping each other's work apart the way art students are. So each week students
answer [three questions](/blog/2026/03/26/comp4020-the-weekly-questions/) in
...
freely without risk. Each week, they work in a fresh, isolated cloud VM
(something like a [Fly.io Sprite](https://fly.io/docs/sprites/))---nothing
personal on it, easy to snapshot, easy to tear down. They run Claude Code in
YOLO mode with no approval prompts, push their code to a class GitLab server
where CI smoke checks verify the basics, and every prototype ends up live on the
open web by default. Peers don't review each other's code---they use each
other's running apps.
...
Nobody really knows the right way to do agentic coding yet. I surveyed
...
field still finding its footing. Part of what excites me about COMP4020 is that
it's a distributed experiment: 100--200 students road-testing these nascent
theories and reporting back on what actually works.

There's still plenty I'm wrestling with. Tool access is the most immediate:
getting ~200 students onto frontier coding agents is a real
...
as a course participant, building prototypes and writing skeets alongside the

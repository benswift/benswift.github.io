...
slide rule operation, and every accumulation a slider moving along a track. The
...
- **Ring D**: weight sliders for the hidden→output connections (6×10 = 60
  weights)
- **Ring E** (innermost): 10 radial sliders for the output (one per digit
  class)
...
<ApparatusInference client:visible />
...
28×28 to 6×6 pixels, it's not going to win any accuracy benchmarks, but
accuracy isn't what it's for: the apparatus exists to make the forward pass
_visible_.
...
- [Interactive neon perceptron visualisation](/blog/2025/12/11/neon-perceptron-digital-twin/):
  a 5×5 input, 9-hidden, 10-output network rendered in Three.js
- [Mini neon perceptron: XOR edition](/blog/2026/03/03/mini-neon-perceptron-xor-edition/):
  a tiny 2×2 input version for testing the physical build

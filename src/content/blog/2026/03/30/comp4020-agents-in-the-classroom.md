...
like Strix and Void that persist and participate in social spaces over time. And
it got me thinking: what if one of them enrolled in COMP4020?

It's a real design question, I think. The course already has all the
...
prototype in response to each provocation and writing its three skeets. It could
even, if we wanted, observe patterns across everyone's work and surface insights
that no individual student or instructor would catch.

The question mark in the title is deliberate; I haven't decided whether to do
...
tools and constraints as the students (Claude Code, a Fly Sprite, the open web);
it would write reflections in response to the same three weekly questions (_why
this?_, _what made it better?_, _any good?_) in the same 280-character skeet
format; it would observe patterns across the cohort's work---which frameworks
are trending, which provocations generated the most diverse responses, where
clusters of similar approaches are forming---and it would maintain persistent
memory across the semester, building up a model of the class's collective
trajectory.
...
Almost nobody else is doing this, as far as I can tell. Which is either a sign
that it's a novel idea or a sign that everyone else has thought better of it.
...
A few things appeal to me about this idea, pedagogically. The agent would act as
both a baseline and a mirror: building a prototype every week using the same
tools and the same provocations, its output becomes a reference point---not a
gold standard, but a data point. How does it interpret the provocation
...
it makes visible what human judgment and context contribute.
...
component library", or "three students independently arrived at similar routing
patterns"---and those observations could seed the studio crit discussions in
useful ways.

And there's a more subtle argument: having an agent in the class is itself a
provocation _about_ the provocations. It forces students to confront what makes
their work different from what an agent produces. If the agent's prototype is
indistinguishable from theirs, that's evidence that the provocation was too
narrow, or that they need to push harder on the dimensions where human judgment
matters.

Set against all that, though, is the obvious risk: the "dead classroom"
scenario, in which the agent participates, the humans disengage, and the course
becomes a performance for an audience of bots. I don't think that's likely with
the studio crit model---you can't fake presence in a face-to-face feedback
session---but it's worth taking seriously.

There's also the question of feedback and assessment. The agent could
technically give students feedback; whether it _should_ is much less clear. The
...
struggled with the same provocation, made the same trade-offs, and has taste and
opinions and blind spots of their own. An agent doesn't struggle. Its feedback
...
reflection show real critical thinking? Those are judgment calls, and I'm not
convinced an agent's judgment is the kind of judgment I want students to
internalise. Not yet, anyway.

If I do build this, the hardest part won't be the infrastructure. It'll be the
SOUL.md, the document that defines who the agent is, what it values, and how it
should behave. Tim Kellogg
...
- transparent about its process: the reflections should say how it worked, what
  it tried, what failed
...
- humble about where its prototypes fall short, especially on taste and user
  experience

But I'd want to write the SOUL.md collaboratively with the students. It's the
kind of design question that would make an excellent provocation in its own
right.

Am I going to do this? Maybe. The course is an experiment in both the future of
software development _and_ tertiary education, and this feels like exactly the
kind of wild idea I should be trying while I have the chance. The realities of
actually running a course for 200 students will hit soon enough, and when they
do, I might decide that a studio agent is one moving part too many.
...
we'll learn something new about how agents and humans can coexist in learning
environments. If it doesn't, well, that's data too. And I'll write about it here
either way.

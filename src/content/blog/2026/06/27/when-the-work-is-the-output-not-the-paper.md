...
what the citable thing was. The rest of the studio's work is messier. _PANIC!_
has two papers attached to it; _LLMs Unplugged_ has one. Once a paper exists,
the temptation is to let the paper be the research output and to file the
installation as the thing the paper is about.
...
on its own. Convenient, and worth keeping. It is also opinionated. Every
automatic deposit comes out typed as _Software_ and licensed _MIT_. For a repo
of code that is exactly right, because a source-zip of the tracker honestly is
software and honestly is MIT. For an installation it is a small lie. _PANIC!_
isn't software. It is an installation that happens to have software inside it,
and its archive shouldn't introduce itself as a code library.
...
The trap I walked into first was assuming Zenodo would read the file I already
had. It doesn't: the GitHub integration reads `.zenodo.json` and ignores
`CITATION.cff`. Several of my repos had `.cff` files sitting in the root,
looking authoritative and controlling precisely nothing about the deposit.[^cff]

The licence is where this gets genuinely subtle, and where being sloppy does
real harm. Once you've decided a thing is "creative", the instinct is to reach
for a Creative Commons licence, and for art the reflex is CC-BY-NC-SA: no
commercial use, share alike. You must never put a non-commercial licence on
code. The NC clause contradicts the open-source grant the code already carries.
MIT says do anything with the code, including sell it; NC says you may not; one
set of files cannot say both. So the rule I settled on is to let the licence
follow the bytes. If the deposit is a source-zip it is Software and it is MIT,
full stop, however creative the work it stands for. CC-BY-NC-SA is reserved for
the deposits that are themselves creative or teaching artefacts: the _LLMs
Unplugged_ books through Cybernetic Studio Press, not the code that typesets
them.

The companion papers get linked, not cited. _PANIC!_ carries related identifiers
pointing at two things: its NIME 2025 proceedings entry, and the
...
files buy me is records that describe themselves accurately: code filed as code,
creative work filed as creative work. The companion papers are linked, not
mistaken for the works they document. The seven works are gathered into a

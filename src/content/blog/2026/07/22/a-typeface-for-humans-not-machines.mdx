...
import ScotomaGenerator from "@/components/svelte/ScotomaGenerator.svelte";
...
Rather than make you clone the repo to try it, here's the renderer in the page.
Type two messages of the same length, and download or copy the result. The blur
is the same three-pass Gaussian approximation the experiment used, so the
numbers below apply to whatever you make here.

<ScotomaGenerator client:visible />

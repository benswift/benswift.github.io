...
(defn verb-component [verb word-order]
  [:h2
   {:style
    (into {:position "fixed"
           :top (str (rand 80) "vh")
           :left (str (+ (* 25 word-order) (rand 25)) "vw")
           :font-weight (+ 100 (rand 800))
           :font-size (str (+ 2 (rand 15)) "vh")}
          (take (rand-int 3)
                {:font-style "italic"
...
(defn billboard-component []
  [:section
   {:data-background-image "https://source.unsplash.com/random/1920x1080"}
   [verb-component (rand-nth verbs) 0]
   [verb-component "your" 1]
   [verb-component (rand-nth verbs) 2]
   [anu-logo-component]])

(defn mount []
  (r/render [billboard-component]
            (js/document.querySelector ".reveal > .slides")))

(mount)

(defn hide-progress-and-page-number []
  (dom/setProperties
   (js/document.querySelector ".progress")
   #js{"display" "none"})
  (dom/setProperties
   (js/document.querySelector ".slide-number")
   #js{"display" "none"}))

;; (hide-progress-and-page-number)

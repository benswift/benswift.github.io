...
BINS = ["none", f"under {cuts[0]}M", f"{cuts[0]}-{cuts[1]}M", f"{cuts[1]}-{cuts[2]}M",
        f"over {cuts[2]}M"]

calendar = spec(
    title={"text": "Every day of agent use",
           "subtitle": "Tokens per day, 3 July 2025 to 19 August 2026. "
                       "Unfilled cells are days with no session at all."},
    data={"values": cal},
    transform=[{"calculate":
                f"datum.tokens === 0 ? '{BINS[0]}'"
    ...
    encoding={
        "x": {"field": "date", "type": "ordinal", "timeUnit": "yearweek",
              "axis": {"title": None, "labelAngle": 0, "grid": False, "domain": False,
                       "ticks": False, "labelOverlap": False, "labelExpr":
                       "date(datum.value) > 7 ? '' : month(datum.value) == 0"
                       " ? timeFormat(datum.value, '%b %y') : timeFormat(datum.value, '%b')"}},
        "y": {"field": "date", "type": "ordinal", "timeUnit": "day",
              "axis": {"title": None, "grid": False, "domain": False, "ticks": False,
                       "format": "%a", "labelExpr":
                       "indexof([1,3,5], day(datum.value)) >= 0 ? timeFormat(datum.value, '%a') : ''"}},
        "color": {"field": "bin", "type": "nominal",
                  "scale": {"domain": BINS, "range": [EMPTY] + RAMP},
                  "legend": {"title": None, "orient": "bottom", "direction": "horizontal",
                             "symbolType": "square", "symbolSize": 130, "columnPadding": 12}},
        "tooltip": [{"field": "date", "type": "temporal", "title": "day"},
                    {"field": "millions", "type": "quantitative", "title": "million tokens",
                     "format": ",.0f"}],
    },
)
...
by_model_daily = spec(
    title={"text": "The rise and fall of each model",
    ...
    transform=[
        {"window": [{"op": "mean", "field": "millions", "as": "smooth"}],
         "frame": [-3, 3], "groupby": ["model"], "sort": [{"field": "date"}]},
    ],
    facet={"row": {"field": "model", "type": "nominal", "sort": model_order, "title": None,
...
cum_rows = []
for r in cum:
    cum_rows.append({"date": r["date"], "measure": "billions of tokens", "value": r["billions"]})
    cum_rows.append({"date": r["date"], "measure": "indicative API cost (US$)", "value": r["dollars"]})

cumulative = spec(
    title={"text": "Cumulative since day zero",
    ...
    facet={"row": {"field": "measure", "type": "nominal",
                   "sort": ["billions of tokens", "indicative API cost (US$)"], "title": None,
                   "header": {"labelAngle": 0, "labelAlign": "left", "labelOrient": "top",
                              "labelPadding": 2}}},
...
when_chart = spec(
    title={"text": "The working day, as seen by the token meter",
    ...
    layer=[
        {"mark": {"type": "line", "strokeWidth": 2, "interpolate": "monotone",
         ...
         "encoding": {"tooltip": [{"field": "kind", "type": "nominal", "title": None},
    ...
    encoding={
        "x": {"field": "hour", "type": "quantitative",
              "axis": {"title": None, "grid": False, "values": [0, 3, 6, 9, 12, 15, 18, 21],
...
for name, s in (("daily-heatmap", calendar), ("rise-and-fall", by_model_daily),
                ("cumulative", cumulative), ("diurnal", when_chart)):

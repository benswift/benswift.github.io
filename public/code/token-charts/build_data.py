...
PRICES = {
    "claude-fable-5": (10, 50), "claude-opus-5": (5, 25), "claude-opus-4-8": (5, 25),
    ...
    "claude-sonnet-4-5-20250929": (3, 15), "claude-sonnet-4-20250514": (3, 15),
    "claude-haiku-4-5-20251001": (1, 5),
}
LABEL = {
    "claude-opus-4-20250514": "Opus 4", "claude-sonnet-4-20250514": "Sonnet 4",
    "claude-opus-4-1-20250805": "Opus 4.1", "claude-sonnet-4-5-20250929": "Sonnet 4.5",
    ...
    "claude-sonnet-5": "Sonnet 5", "claude-opus-5": "Opus 5", "claude-fable-5": "Fable 5",
}
...
for d in all_days:
    ...
    cal.append({"date": key, "week": week_start.isoformat(), "weekday": d.weekday(),
                "tokens": tok, "millions": round(tok / 1e6, 1)})
...
for d in all_days:
    ...
    cum_t += t
    cum_c += c
    cum.append({"date": d.isoformat(), "billions": round(cum_t / 1e9, 4),
...
for m in models:
    for d in all_days:
        ...
        per_model.append({"date": key, "model": m, "millions": round((v[0] if v else 0) / 1e6, 2)})
json.dump(per_model, open("per_model_daily.json", "w"), indent=0)
json.dump(models, open("model_order.json", "w"))

# --- when: weekday x hour
WD = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"]
json.dump([{"weekday": WD[wd], "hour": h, "millions": round(when.get((wd, h), 0) / 1e6, 1)}
           for wd in range(7) for h in range(24)], open("when.json", "w"), indent=0)

busiest = sorted(daily.items(), key=lambda kv: -kv[1][0])[:5]
print(f"span {start} .. {end} ({len(all_days)} days), {sum(1 for d in daily if daily[d][0])} active")
...
json.dump(
    [{"kind": k, "hour": h,
      "millions": round(prof.get((k, h), 0) / (n_weekday if k == "Mon-Fri" else n_weekend) / 1e6, 2)}
     for k in ("Mon-Fri", "Sat-Sun") for h in range(24)],
    open("diurnal.json", "w"), indent=0)
